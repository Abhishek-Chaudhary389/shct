...
import { getBetiSahyogReceipts, getApprovedMembers, getBetiSahayogList } from '../../services/dataService';

const BetiAccountHolderWiseList = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const approvedReceipts = rawReceipts
          ...
          .map(r => {
            ...
            return {
              ...
              beneficiaryName: (r.beneficiaryName || r.donationToMember || 'सामान्य सहायता').trim(),
        ...
        applications.forEach(app => {
          ...
          if (name) {
            holderMap[name] = {
              ...
              vivahDate: app.marriageDate || app.date || '2025-04-15',
        ...
        console.error("Error loading account holder wise data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  // Open Detail View for a specific Account Holder
      ...
      >
        <div className="text-center z-10 px-4">
          <h1 className="text-3xl md:text-5xl font-black text-white tracking-wide uppercase">
            {selectedHolder ? `Beti Vivah Sahyog - ${selectedHolder.name}` : 'Beti Vivah Sahyog - Account Holder Wise'}
          </h1>
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                <div>
                  ...
                  <input
                    ...
                    placeholder="जिला टाइप करें (e.g. Gorakhpur)"
                    className="w-full px-3.5 py-2.5 bg-gray-50 border border-gray-300 rounded-xl focus:ring-2 focus:ring-[#087889] outline-none"
                  />
                </div>
                <div>
                ...
                <div className="overflow-x-auto border border-gray-200 rounded-2xl">
                  <table className="w-full text-left border-collapse min-w-[900px] text-sm">
                    <thead>
                      <tr className="bg-gray-50 border-b border-gray-200 text-gray-700 font-extrabold uppercase text-[11px] tracking-wider">
                        <th className="py-4 px-5">S NO</th>
                        <th className="py-4 px-5">Name (खाताधारक)</th>
                        <th className="py-4 px-5">Amount</th>
                        <th className="py-4 px-5">District</th>
                        <th className="py-4 px-5">Block</th>
                        <th className="py-4 px-5">Vivah Sahyog Date</th>
                      ...
                      {currentHolders.length === 0 ? (
                        ...
                        currentHolders.map((holder, idx) => (
                          <tr key={idx} className="hover:bg-teal-50/40 transition-colors">
                            ...
                            <td className="py-4 px-5 text-gray-500 font-semibold">{holder.vivahDate}</td>
...
export default BetiAccountHolderWiseList;
