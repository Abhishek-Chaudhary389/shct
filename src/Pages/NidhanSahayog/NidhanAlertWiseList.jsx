...
import { getNidhanSahyogReceipts, getApprovedMembers, getNidhanSahayogList, getHomeAlerts } from '../../services/dataService';

const NidhanAlertWiseList = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const approvedReceipts = rawReceipts
          ...
          .map((r) => {
            ...
            return {
              ...
              beneficiaryName: (r.beneficiaryName || r.donationToMember || r.deceasedName || '').trim(),
        ...
        console.error("Error loading nidhan alert wise data:", err);
      } finally {
        setLoading(false);
      }
    };
      ...
      >
        <div className="text-center z-10 px-4">
          <h1 className="text-3xl md:text-5xl font-black text-white tracking-wide uppercase">
            {selectedAlert ? `Nidhan Sahyog List - ${selectedAlert.title}` : 'Alert Wise Death Sahyog'}
          </h1>
            ...
            {loading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                {alertsList.map((alert) => (
                  ...
                  >
                    {/* Top Accent line */}
                    <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-slate-700 to-[#f08519] opacity-0 group-hover:opacity-100 transition-opacity"></div>
                    ...
                    <div className="my-2">
                      ...
                      {alert.beneficiaryName && (
                        <p className="text-xs font-bold text-slate-700 bg-slate-100 px-2.5 py-0.5 rounded-md inline-block">
                          {alert.beneficiaryName}
                        </p>
                      )}
                    </div>
...
export default NidhanAlertWiseList;
